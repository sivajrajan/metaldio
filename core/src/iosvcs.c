...
/*
 * dsdd_alloc_autoclose - allocate a dataset DD with DALCLOSE.
 *
 * DALCLOSE (SVC 99 text unit key 0x0058) is a zero-length text unit that
 * instructs MVS to automatically unallocate the DD when the DCB opened
 * against it is closed via the CLOSE macro.  After CLOSE completes, the DD
 * is gone and no explicit SVC 99 UNFREE is needed.  This eliminates the
 * S99ERROR:0x03A8 failure that occurs when DYNFREE is attempted on a DD that
 * was OPENed with BPAM.
 */
