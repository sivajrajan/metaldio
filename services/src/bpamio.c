...
static int alloc_pds(const char* dataset, FM_BPAMHandle* bh, const DBG_Opts* opts)
{
  ...
  /*
   * Use DALDDNAM (0x01) with an explicit generated name instead of
   * DALRTDDN (0x55).  DALRTDDN places the DD in the XTIOT; SVC 99
   * UNFREE (S99VRBUN/DUNDDNAM) on an XTIOT entry returns error:0x03A8
   * even after CLOSE because the XTIOT tracks a separate internal DCB
   * reference that CLOSE TYPE=T does not clear.  DALDDNAM allocates
   * into the classic TIOT where DYNFREE works correctly after CLOSE.
   */
